(function(app) {
    ...
    function startApplication(){
        ...
        oContentLoader.evts.addEventListener("AN_OVERVIEW_OF_THE_GUIDELINES_CLICKED", function(a,b,c){
            if(c==true){
                screen3_flag = true;
            }
            setEnableMenu();
         ...
         oContentLoader.evts.addEventListener("APPORTIONING_PERSONAL_RESPONSIBILITY_FOR_THE_OCCURRENCE_CLICKED", function(a,b,c){
            if(c==true){
                screen6_flag = true;
            }
            setEnableMenu();
         ...
         oContentLoader.evts.addEventListener("SCOPE_AND_PURPOSE_OF_THE_BOI_TRACKER_CLICKED", function(a,b,c){
            if(c==true){
                screen8_flag = true;
            }
            setEnableMenu();
            ...
            }else if(c == "Glossary")
            {
                ...
                $(document).ready(function() {
                    $.getJSON( "assets/json/config.json", function( data ) {
                        $('.menuitem_1, .menuitem_2, .menuitem_3, .menuitem_4, .menuitem_5, .menuitem_6, .menuitem_7, .menuitem_8, .menuitem_9, .menuitem_10').unbind('keypress').bind('keypress', function(e){
    ...
    function setSuspendedData(){
        suspendData = nCurrentPage+"<||>"+screen1_flag+"<||>"+screen2_flag+"<||>"+screen3_flag+"<||>"+screen4_flag+"<||>"+screen5_flag+"<||>"+screen6_flag+"<||>"+screen7_flag+"<||>"+screen8_flag+"<||>"+screen9_flag+"<||>"+screen10_flag;
    ...
    function setNextEnable(){
        if(screen1_flag == true && nCurrentPage==1){
            ...
            $('a[menu-data = '+'Next'+']').addClass('disabled');
        }else if(screen10_flag == false && nCurrentPage==10){
            $('a[menu-data = '+'Next'+']').addClass('disabled');
        }else{
            setTimeout(function(){
                $('a[menu-data = '+'Next'+']').addClass('disabled');  
    ...
    $(document).ready(function() {
        $.getJSON( "assets/json/config.json", function( data ) {
            $('.access_anchor_back, .access_anchor_next').unbind('keypress').bind('keypress', function(e){
                if(e.type === 'keypress' && e.keyCode != 13 && e.keyCode != 32 && e.keyCode != 9){
                    return false;
                }
                if(e.keyCode !=9){
