(function(app) {
    ...
    function startApplication(){
        ...
        oContentLoader.evts.addEventListener("BOI_REPORT_CLICKED", function(a,b,c){
            if(c==true){
                screen3_flag = true;
            }
            setEnableMenu();
        ...
        oContentLoader.evts.addEventListener("SMART_CRITERIA_CLICKED", function(a,b,c){
            if(c==true){
                screen4_flag = true;
            }
            setEnableMenu();
        ...
        oContentLoader.evts.addEventListener("FORMULATING_SMART_RECOMMENDATIONS_CLICKED", function(a,b,c){
            if(c==true){
                screen5_flag = true;
            }
            setEnableMenu();
        ...
        document.addEventListener('swiped-left', function(e) {
            if(nCurrentPage <  nTotalPages-1){
                ...
                // counter++;
                setPageNumber();
                _html.find('.transcript_data_mod7').addClass("hide");
                transcriptOn = false;
                // loadPage();
    ...
    function createNavigation(){
        ...
        oNavigation.evts.addEventListener("NAV_CLICKED",function(a,b,c){
            ...
            if(!isNaN(c) && !$('a[menu-data = '+c+']').hasClass("disabled"))
            {
                ...
                $('.close_text_area_mod7').addClass("hide");
                ...
                loadPage()
                if(screen6_flag == false && nCurrentPage==6){
                    setTimeout(function(){
            ...
            }else if(c == "Glossary")
            {
                ...
                $(document).ready(function() {
                    $.getJSON( "assets/json/config.json", function( data ) {
                        $('.menuitem_1, .menuitem_2, .menuitem_3, .menuitem_4, .menuitem_5, .menuitem_6, .menuitem_7').unbind('keypress').bind('keypress', function(e){
    ...
    function setSuspendedData(){
        suspendData = nCurrentPage+"<||>"+screen1_flag+"<||>"+screen2_flag+"<||>"+screen3_flag+"<||>"+screen4_flag+"<||>"+screen5_flag+"<||>"+screen6_flag+"<||>"+screen7_flag;
    ...
    function setNextEnable(){
        if(screen1_flag == true && nCurrentPage==1){
            ...
            $('a[menu-data = '+'Next'+']').addClass('disabled');
        }else if(screen7_flag == false && nCurrentPage==7){
            $('a[menu-data = '+'Next'+']').addClass('disabled');
        }else{
            setTimeout(function(){
                $('a[menu-data = '+'Next'+']').addClass('disabled');  
    ...
    function activateAudioPlayer(){
        $('.click_cursor_unmute, .click_cursor_mute, .click_cursor_play, .click_cursor_pause, .click_cursor_replay, .click_cursor_text, .close_text_area_mod7').on('keypress', function(e){
            if(e.type === 'keypress' && e.keyCode != 13 && e.keyCode != 32 && e.keyCode != 9){
                return false;
            } 
            if(e.keyCode !=9){
      ...
      _html.find('.click_cursor_text').click(function(){
            if(!transcriptOn){
                $('.transcript_data_mod7  > p').html('');
                $('.transcript_data_mod7').removeClass("hide");
                $('.transcript_data_mod7 > p').append(jsonData.pages[nCurrentPage].data.transcript);
                $('.close_text_area_mod7').removeClass("hide");
        ...
        $('.close_text_area_mod7').click(function(){
            $('.transcript_data_mod7').addClass("hide");
            $('.close_text_area_mod7').addClass("hide");
            $('.audio__control').removeClass("hide");
            transcriptOn = false;
