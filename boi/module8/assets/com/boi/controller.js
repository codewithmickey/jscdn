(function(app) {
    ...
    function startApplication(){
        ...
        oContentLoader.evts.addEventListener("MAINTAINING_THE_BOI_TRACKER_CLICKED", function(a,b,c){
            if(c==true){
                screen4_flag = true;
            }
            setEnableMenu();
        ...
        oContentLoader.evts.addEventListener("DISSEMINATION_AND_RELEASE_OF_REPORTS_CLICKED", function(a,b,c){
            if(c==true){
                screen5_flag = true;
            }
            setEnableMenu();
        ...
        document.addEventListener('swiped-left', function(e) {
            if(nCurrentPage <  nTotalPages-1){
                ...
                // counter++;
                setPageNumber();
                _html.find('.transcript_data_mod8').addClass("hide");
                transcriptOn = false;
                // loadPage();
            }
        });
    ...
    function createNavigation(){
        ...
        oNavigation.evts.addEventListener("NAV_CLICKED",function(a,b,c){
            ...
            if(!isNaN(c) && !$('a[menu-data = '+c+']').hasClass("disabled"))
            {
                ...
                $('.transcript_data_mod8').addClass("hide");
                $('.close_text_area_mod8').addClass("hide");
    ...
    function createNavigationAfterReviewModule(){
        ...
        oNavigation.evts.addEventListener("NAV_CLICKED",function(a,b,c){
            ...
            }else if(c == "Glossary")
            {
                ...
                // $('.nextback').addClass("hide");
                $("#exampleModal").click("hidden.bs.modal", function () {
                    $('.nextback').removeClass("hide");
                    $('.current_glossary_key').removeClass("current_glossary_key");
                });
                setPageNumber();
    ...
    function activateAudioPlayer(){
       ...
       $(document).ready(function() {
        ...
        _html.find('.click_cursor_text').click(function(){
            if(!transcriptOn){
                $('.transcript_data_mod8  > p').html('');
                $('.transcript_data_mod8').removeClass("hide");
                $('.transcript_data_mod8 > p').append(jsonData.pages[nCurrentPage].data.transcript);
                $('.close_text_area_mod8').removeClass("hide");
        ...
        $('.close_text_area_mod8').click(function(){
            $('.transcript_data_mod8').addClass("hide");
            $('.close_text_area_mod8').addClass("hide");
            $('.audio__control').removeClass("hide");
            transcriptOn = false;
